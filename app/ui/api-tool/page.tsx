...
function ApiTool() {
  const { messages, sendMessage, status, error, stop } = useChat<ChatMessages>({
    ...
    onError: async (error) => {
      toast.error("API tool error occurred: " + error.message);
    },
  });
            ...
            {messages.length === 0 ? (
              <div className="flex items-center justify-center h-full px-4 py-10">
                <div className="text-center">
                  ...
                  <p className="text-gray-500">
                    Ask about weather in any city around the world. Get
                    real-time weather data with beautiful visualizations!
                  </p>
                  <div className="mt-6 space-y-3">
                    <div className="text-xs font-medium text-gray-400 uppercase tracking-wide">
                      Try these weather examples
                    </div>
              ...
              <div className="px-4 py-6 space-y-6">
                {messages.map((message) => (
                  ...
                  >
                    <div
                      className={`w-full lg:max-w-md px-4 py-3 rounded-2xl shadow-lg transition-all duration-200 hover:shadow-xl ${
                        message.role === "user"
